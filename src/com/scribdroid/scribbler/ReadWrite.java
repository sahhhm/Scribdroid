...
public class ReadWrite {
    ...
    public static void _write(BluetoothSocket sock, boolean connected,
            byte[] values) {
        if (connected) {
    ...
    public static void _writeFluke(BluetoothSocket sock, boolean connected,
            byte[] values) {
        if (connected) {
            OutputStream out = null;
            try {
    ...
    public static byte[] _read(BluetoothSocket sock, Boolean connected,
            int numBytes) {
        ...
        if (connected) {
            ...
            try {
                while (buf.hasRemaining()) {
                    ...
                    /*
                     * if (read - buf.remaining() < 0) { for (int i = 0; i <
                     * read - buf.remaining(); i++) { int b = buffer[i] & 0xff;
                     * buf.put((byte) b); } Log.d(TAG, "in BREAK"); break; }
                     */
