            ...
            public void onClick(View v) {
                if (sdAvailable && selectedPosition != -1) {
                    String path = Environment.getExternalStorageDirectory()
                            .toString() + File.separator + selectedName;
                    FileOutputStream out;
                    try {
                        Bitmap bmp = BitmapFactory.decodeFile(getFilesDir()
                                .toString()
                                + File.separator
                                + fileList()[selectedPosition].toString());
                        out = new FileOutputStream(path);
                        bmp.compress(Bitmap.CompressFormat.JPEG, 90, out);
                        Toast.makeText(
                                getBaseContext(),
                                getResources().getString(
                                        R.string.success_external_save),
    ...
    public class ImageAdapter extends BaseAdapter {
        ...
        public ImageAdapter(Context c) {
            ...
            mGalleryItemBackground = attr.getResourceId(
                    R.styleable.PictureGallery_android_galleryItemBackground,
                    0);
            attr.recycle();
        }
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            imageView.setImageBitmap(BitmapFactory.decodeFile(getFilesDir()
                    .toString()
                    + File.separator
                    + fileList()[position].toString()));
